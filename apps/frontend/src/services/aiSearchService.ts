...
export const semanticSearchExperts = async (query: string, experts: Expert[]): Promise<ScoredExpert[]> => {
  try {
...
Important: Include ALL experts in your response, even if their score is 0.0.`;
    ...
    const scoredExperts: ScoredExpert[] = scores
      .map(scoreData => ({
...
const fallbackKeywordSearch = (query: string, experts: Expert[]): ScoredExpert[] => {
...
export const analyzeSearchQuery = async (query: string): Promise<QueryAnalysis> => {
  try {
...
Respond with ONLY a JSON object in this format:
{
  ...
  "searchTerms": ["key", "terms", "extracted"]
    ...
    return JSON.parse(responseText) as QueryAnalysis;
  } catch (error) {

...
import { Database, Zap, CheckCircle, AlertCircle, Loader, Search, Brain, Users, Award, Bug } from 'lucide-react';
...
import { runExpertVectorDiagnostics, formatDiagnosticReport, getQuickStatus } from '../services/diagnostics.js';
...
const ExpertVectorUploader = () => {
    ...
    try {
      ...
      if (tableExists) {
        try {
          ...
          console.error('Error getting vector stats:', statsError);
          setVectorStats(null);
        }
      }
  ...
  const handleRunDiagnostics = async () => {
    ...
    try {
      const diagnosticResults = await runExpertVectorDiagnostics();
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        ...
        {loading && !diagnostics && (
          <div className="text-center py-4">
            <Loader className="h-6 w-6 animate-spin mx-auto mb-2 text-yellow-600" />
            <p className="text-yellow-700">Running diagnostics...</p>
          </div>
        )}
        ...
        {diagnostics && (
          <div className="space-y-3">
            <div className="flex items-center space-x-4 text-sm">
              <span className="text-green-600 font-medium">{diagnostics.summary.passed} Passed</span>
              <span className="text-red-600 font-medium">{diagnostics.summary.failed} Failed</span>
              <span className="text-yellow-600 font-medium">{diagnostics.summary.warnings} Warnings</span>
            </div>
            
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {diagnostics.tests.map((test, index) => (
                <div key={index} className={`p-3 rounded border ${
                  test.status === 'PASS' ? 'bg-green-100 border-green-200' :
                  test.status === 'FAIL' ? 'bg-red-100 border-red-200' :
                  'bg-yellow-100 border-yellow-200'
                }`}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h4 className={`font-medium ${
                        test.status === 'PASS' ? 'text-green-800' :
                        test.status === 'FAIL' ? 'text-red-800' :
                        'text-yellow-800'
                      }`}>
                      ...
                      {test.details?.solution && (
                        <p className="text-xs mt-1 font-medium text-blue-600">
                          Solution: {test.details.solution}
                        </p>
                      )}
                    </div>
                  </div>

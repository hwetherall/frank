...
function createExpertScoringPrompt(profile) {
  ...
  if (profile.ld_position && profile.ld_position !== profile.position) positions.push(`Headline: ${profile.ld_position}`);
  const positionText = positions.length > 0 ? positions.join('; ') : 'Not specified';

  // Company analysis
  const companies = [];
  if (profile.ld_company) companies.push(profile.ld_company);
  if (profile.current_company && typeof profile.current_company === 'string' && profile.current_company !== profile.ld_company) {
    companies.push(profile.current_company);
  } else if (profile.current_company?.name && profile.current_company.name !== profile.ld_company) {
    companies.push(profile.current_company.name);
  }
